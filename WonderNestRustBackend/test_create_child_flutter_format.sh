...
# Test the create child endpoint with Flutter format (empty interests array)

BASE_URL="http://localhost:8080/api/v1"
EMAIL="test$(date +%s)@example.com"
PASSWORD="TestPass123!"

echo "Testing child creation with Flutter format (empty interests)..."
echo "Email: $EMAIL"
echo ""
...
# 2. Test child creation with Flutter format (empty arrays/strings)
echo "2. Creating child profile with Flutter format..."
...
if echo "$CHILD_RESPONSE" | grep -q "success.*true"; then
  echo "✅ Child created successfully with Flutter format!"
else
